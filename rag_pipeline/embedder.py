...
class Embedder(Protocol):
    ...
    def doc_vectors(self) -> np.ndarray:
        ...
        Each row must be L2-normalised so cosine similarity reduces to a
...
class TFIDFEmbedder:
    ...
    def fit(self, texts: list[str]) -> None:
        ...
        normalised = normalize(sparse, norm="l2")
...
class SentenceTransformerEmbedder:
    ...
    def fit(self, texts: list[str]) -> None:
        log.info(f"SentenceTransformerEmbedder: pre-encoding {len(texts)} texts")
...
def cosine_similarity(vec_a: list[float], vec_b: list[float]) -> float:
    ...
    All embedders produce L2-normalised vectors so this is just a dot product.
    Explicit norm division retained as a safety fallback for unnormalised inputs.
    """
